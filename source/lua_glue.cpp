...
HWND DetermineTargetWindow(LPCTSTR aTitle, LPCTSTR aText, LPCTSTR aExcludeTitle, LPCTSTR aExcludeText)
{
	TCHAR _aTitle[256];
	TCHAR _aText[256];
	TCHAR _aExcludeTitle[256];
	TCHAR _aExcludeText[256];

	_tcscpy(_aTitle, aTitle);
	_tcscpy(_aText, aText);
	_tcscpy(_aExcludeTitle, aExcludeTitle);
	_tcscpy(_aExcludeText, aExcludeText);
	...
	IF_USE_FOREGROUND_WINDOW(g->DetectHiddenWindows, aTitle, aText, aExcludeTitle, aExcludeText)
		...
		target_window = WinExist(*g, _aTitle, _aText, _aExcludeTitle, _aExcludeText);
...
{
	LUA_ARG_CONST_STRING( Title, 1 );
	LUA_ARG_CONST_STRING( Text, 2 );
	LUA_ARG_CONST_STRING( ExcludeTitle, 3 );
	LUA_ARG_CONST_STRING( ExcludeText, 4 );
...
int lua_Gui(lua_State* L)
{
	LUA_ARG_STRING( Buf, 1 );
	LUA_ARG_STRING( ControlType, 2 );
	LUA_ARG_STRING( Options, 3 );
	LUA_ARG_STRING( Param4, 4 );

	ResultType ret = g_script.PerformGui( tszBuf, tszControlType, tszOptions, tszParam4 );
...
int lua_Sleep(lua_State* L)
{
	...
	// documentation.
	if (nSleepTime  < 25 && nSleepTime > 0 && g_os.IsWin9x()) // Ordered for short-circuit performance. v1.0.38.05: Added "sleep_time > 0" so that Sleep -1/0 will work the same on Win9x as it does on other OSes.
		Sleep(nSleepTime);
	else
		MsgSleep(nSleepTime);
